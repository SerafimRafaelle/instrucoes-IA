...
Quando houver conflito entre atividades ou regras, aplicar a seguinte ordem:
...
Quando houver estudo efetivo e a usuária solicitar o processo de revisão, aplicar o ciclo:
...
As revisões devem priorizar recuperação ativa, exercícios, aplicação, resolução de problemas, identificação de lacunas e correção de erros.



Revisões não devem ser criadas apenas porque uma ferramenta foi instalada ou configurada; deve existir estudo efetivo suficiente.
...
A precedência ocorre dentro de cada eixo:
...
Não existe limite máximo fixo de revisões de vestibular por dia. A quantidade deve respeitar tempo disponível, duração, conflitos, carga cognitiva, energia e sustentabilidade.
...
Não se deve empilhar tarefas mecanicamente no dia seguinte. A atividade deve ser avaliada para decidir se será:
...
A qualidade da rotina prevalece sobre compensações artificiais.
...
Quando a alteração não tiver sido explicitamente autorizada, apresentar uma proposta antes da execução.



Após qualquer alteração, verificar novamente conflitos, duplicidades, datas, horários, duração, pausas, revisões e preservação dos demais compromissos.
...
As regras deste documento representam a visão resumida das regras fundamentais.
...
Em caso de alteração das regras, a documentação deve permanecer coerente e não deve haver aplicação simultânea de regras contraditórias.
